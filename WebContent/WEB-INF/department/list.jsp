	...
	<form action="RoleController?service=show&department_id" method="post">
		<span class="span">
			...
			搜索字段:<select name="fuzzyQuery" style="font-size:12px; ">
					...
					<option value="department_desc">部门描述</option>
					<option value="status">部门状态</option>
    ...
    <thead>   
        <tr >              
           <th data-options="field:'number'" width="80">编号</th>      
            <th data-options="field:'department_id'" width="80">部门ID</th>     
            <th data-options="field:'department_name'" width="100">部门名称</th>   
            <th data-options="field:'department_desc'" width="140">部门描述</th>   
    ...
    <tbody>   
		<c:forEach begin="0" end="${list.size()-1}" var = "i">
			<tr>
				...
				<td>
				<a target="iframe_a" href="RoleController?service=update&department_id=${list[i].department_id}" onclick="return update(${list[i].status})" ><img src="easyui/themes/icons/pencil.png"></a>&nbsp;&nbsp;&nbsp;&nbsp;
			    <a  href="RoleController?service=delete&department_id=${list[i].department_id}" onclick="return deleteUser(${list[i].status})"><img src="easyui/themes/icons/cancel.png"></a>
	...
	<div  style="height:9%; width:102%;position: fixed;bottom:0; background: #FFFFFF;">
		...
		<div style="background:#FFFFFF;height:100px; width:20%;position:absolute ;top:0%;right: 0%; ">
			...
			<li>
				<c:choose>
					<c:when test="${page!=1}">
						<a href="RoleController?service=show&department_id&page=${page-1}">上一页</a>
					</c:when>
					<c:otherwise>
				...
				<c:choose>
					...
					<a href="RoleController?service=show&department_id&page=${page+1}">下一页</a>
					</c:when>
					<c:otherwise>
