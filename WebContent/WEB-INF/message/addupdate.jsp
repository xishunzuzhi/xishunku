...
<h4>发件邮件</h4>			
	<table border="1" style="border-collapse:collapse; border-color: #DCDCDC">
		...
		<tr>
			<td class="td">发件人 </td>
			<td class="td1">&nbsp;<input type="text" id="sender1" name="sender" value="${user_id}" readonly placeholder="请填写添加人 "  > </td>
			<td class="td">接收人 </td>
			<td class="td1">&nbsp;
			<select id="receiver1" id="receiver1" name="receiver"style="font-size:12px; height: 25px; width: 70px;"></select> 
			</td>
		</tr>
		...
		<tr>
			...
			<input type="hidden" id="message_id1" name="message_id" value="${message.message_id}"/>
...
<script type="text/javascript">
	...
	function on(){
		...
		$.post("NoticeController?service=showMessage&addMessage",
			{"message_id":$("#message_id1").val(),"subject":$("#subject1").val(),"sender":$("#sender1").val(),"receiver":$("#receiver1").val(),"content":$("#content1").val()},
		function(){
				location.href="NoticeController?service=queryDrafts&addMessage=0";
