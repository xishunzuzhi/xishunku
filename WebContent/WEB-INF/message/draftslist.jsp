...
<body >
	...
	<a target="iframe_a" href="RoleController?service=addUser&department_id"  class="easyui-linkbutton" style=" background:#228fbd;color:#C0C0C0;position:absolute ;top:45px;right: 10px">添加草稿箱</a>  
		...
		<c:choose>
			...
			<c:otherwise>
				<tr>
					...
					<td>
						<a target="iframe_a" href="NoticeController?service=updateMessage&id=${list[i].message_id}" onclick="return update()" ><img src="easyui/themes/icons/pencil.png"></a>&nbsp;&nbsp;&nbsp;&nbsp;		
	...
	<div  style="height:9%; width:102%;position: fixed;bottom:0; background: #FFFFFF;">
		...
		<div style="background:#FFFFFF;height:100px; width:20%;position:absolute ;top:0%;right: 0%; ">
			...
			<li>
				<c:choose>
					<c:when test="${page!=1}">
						<a href="NoticeController?service=queryDrafts&addMessage=true&page=${page-1}">上一页</a>
					</c:when>
					<c:otherwise>
				...
				<c:choose>
					...
					<a href="NoticeController?service=queryDrafts&addMessage=true&page=${page+1}">下一页</a>
					</c:when>
					<c:otherwise>
