...
<body >
    ...
    <tbody>   
		<c:forEach begin="0" end="${list.size()-1}" var = "i">
			...
			<c:choose>
					<c:when test="${list[i].receive_delete==0}"></c:when>
					<c:otherwise>
						<tr>
					<td>${i+1}</td>
					...
					<td>
						<a target="iframe_a" href="NoticeController?service=show&addMessage=${list[i].message_id}" onclick="return update()" ><img src="easyui/themes/icons/Mail_Receive.png"></a>&nbsp;&nbsp;&nbsp;&nbsp;
			        	<a href="NoticeController?service=delete&addMessage&message_id=${list[i].message_id}" onclick="return deleteUser()"><img src="easyui/themes/icons/delete_folder.png"></a>
					</td>
				</tr>
			</c:otherwise>
		</c:choose>
	...
	<div  style="height:9%; width:102%;position: fixed;bottom:0; background: #FFFFFF;">
		...
		<div style="background:#FFFFFF;height:100px; width:20%;position:absolute ;top:0%;right: 0%; ">
			...
			<li>
				<c:choose>
					<c:when test="${page!=1}">
						<a href="NoticeController?service=show&addMessage&page=${page-1}">上一页</a>
					</c:when>
					<c:otherwise>
				...
				<c:choose>
					...
					<a href="NoticeController?service=show&addMessage&page=${page+1}">下一页</a>
					</c:when>
					<c:otherwise>
